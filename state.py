...
class State(object):
    ...
    # Broji zatvorene ili otvorene mice
    def count_mills(self, token, n):
        counter = 0
        mills = self.get_mills()
        for mill in mills:
            temp = [self._board[mill[0]], self._board[mill[1]], self._board[mill[2]]]
    ...
    def is_end(self):
        if self._max_pieces == 2 or self._min_pieces == 2:
            ...
            if self.get_all_moves('@') == [] or self.get_all_moves('#') == []:
                return True
        return False
    ...
    def evaluate(self):
        mill_diff = self.count_mills(self._MAX_TOKEN, 3) - self.count_mills(self._MIN_TOKEN, 3)
        move_diff = len(self.get_all_moves(self._MAX_TOKEN)) - len(self.get_all_moves(self._MIN_TOKEN))
        two_piece_diff = self.count_mills(self._MAX_TOKEN, 2) - self.count_mills(self._MIN_TOKEN, 2)
        token_diff = self._max_pieces - self._min_pieces

        if self._stage == 1:
            evaluation =  26 * mill_diff + move_diff + 10 * two_piece_diff + 9 * token_diff
        else:
            evaluation = 14 * mill_diff + 10 * 3 + 11 * token_diff + 1086 * self.winner()
        return evaluation
    ...
    def get_all_moves(self, token=None):
        moves = []
        if self._stage == 1:
            available_points = self.get_all_tokens('x')
            for i in available_points:
                moves.append((None, i))
        else:
            token_keys = self.get_all_tokens(token)
            for i in token_keys:
                for j in self._adjacent_points[i]:
                    if self._board[j] == 'x':
                        moves.append((i, j))
        return moves
